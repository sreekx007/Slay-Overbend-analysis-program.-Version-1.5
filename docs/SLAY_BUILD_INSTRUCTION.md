...
| T9 | L3/L5 | thicken: ILS placement, component contact ownership | **M2–M4** | 🎯 21 Sep 2026 — target set: GD-TP, GD-SH, GD-TP+GD-SH (paper Series 3, 4, 5). Target register in `docs/modules/T9_components_spec.md`. GD-TP is Phase 1 and reachable now; GD-SH and the combination are **Phase 2 in every row** and gated on T6's sweep (L055). Geometry convention CONFIRMED to match: the paper thickens outward at constant bore (OD 471 mm, ~3.2x stiffness for the 53 mm case) and `ThickPipeBody` computes 470.4 mm and 3.248x. GD-TP unblocked (L054) |
...
| 14 Sep 2026 | **Two corrections from the PS run.** (1) The prescribed-stiffness connector was not self-equilibrating — a beam matrix is only valid when its `L` is its own geometry's, and building at `L = OD` while spanning 0.6096 m left ILS-EAST 146 kN·m short of moment equilibrium. Rebuilt on the 3-DOF corotational basis: constitutive law at `L = OD`, kinematics at the real length. (2) **`S` is a roller, not a prismatic pair** — it was tying rz and carrying a pure 338.79 kN·m couple, which a bolt in a slot cannot provide. `P` and `S` are now a pin and a roller, both moment-free. Consequence: under PS the connectors carry nothing and the ILS is a **passenger** — the pipe deflects exactly as a bare pipe, where F2 stiffens it 24.5 %. |
| 14 Sep 2026 | **PS run on the complete ILS-EAST model** — same geometry and mesh, only the tied DOF differ. `P` carries zero moment at both ends (a true two-force member), `S` carries equal and opposite end moments (zero shear). PS barely shields the pipe where F2 drops it to 64 MPa: **2.5× on the pipe's stress in the ILS region from the joint type alone**, and the frame bends 6.9× harder because only one slot ties rotation. G9 unbreached — `build_model` still refuses P/S/D, and the override is global-axis, exact only because this rig is horizontal. |
| 14 Sep 2026 | **ILS-EAST solved end to end** (`tools/study_east_full.py`, 14 tests) — pipe, frame and connectors as one model, both associations enforced as penalty constraints. The ILS stiffens the pipe 24 % against a bare beam of the same span, and shields it between the connector stations (170 → 64 MPa) with the concentrations just outside. Bug found: `user_node_to_mesh` is a **permutation, not the identity** — the kernel numbers nodes in element-encounter order, so writing stiffness at `3*node_id` went to the wrong rows and produced an exactly singular matrix. Every caller must route DOFs through the map. |
| 13 Sep 2026 | **First working penalty constraints** (`tools/study_connectors.py`, 26 tests). GD-ST tied through real connectors on all five named layouts: F1, F2, PS, PSD, F2D. Penalty scaled to the **local** diagonal rather than the kernel's global max, and α = 1e5 chosen by sweep — displacement converged, violation 0.5 nm, cond(K) 3.9e13. Two findings: an engaged deadband must hold AT the gap edge, not at zero (enforcing zero made the active set chatter, four flips, never settling); and PSD/F2D are bit-identical to PS/F2 until a gap closes, so the D path needed its own sweep to be exercised at all. Prescribed-stiffness connector element prototyped — the thing §8 says L5 needs. Still global-axis, not co-rotating. |
| 13 Sep 2026 | Connector stiffness rule restated as **absolute**: every connector carries the stiffness of a 1 × OD length of pipeline whatever its own length, so a zero-length connector is an ordinary case and an ordinary element. Corrects an earlier claim that it was no element at all. All three Group B archetypes now carry connector elements. Confirms L5 needs a **prescribed-stiffness element type** — the corotational kernel divides by the deformed length and returns inf for a zero-length element (`nlfea_v4.py:1216`, observed). |
| 13 Sep 2026 | **The rig SOLVED through `build_model`** (`tools/run_mesher_rig.py`, 23 tests). Until this, nothing from the assembly had been through the solver. Every deflection reproduces the test plan's §5 figures to the last digit — and those were measured on a hand-built mesh before `build_model` existed, so two independent construction paths agree to nine significant figures. Peak stress and strain plotted per component. Added `build_model(extra_stations=...)`: a load needs a node under it and ILS-SH has no structural node at its own centre. |
