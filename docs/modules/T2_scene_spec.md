...
## 9. FINDING FROM THE DRAWING — the end zones swallow the rollers

The layout diagram was generated from Scene rather than drawn, and it made
visible something the prose did not.

With the model extent taken as the station span (`margin = 0`), the 16 m
forced-elastic end zones reach **inward past six of the twelve stations**:

| margin | extent | zones | stations inside a zone |
|---|---|---|---|
| 0 m | −40 … +48 | (−40,−24) (32,48) | VR5, VR4, VR3, **SR5, SR6**, SR7 |
| 16 m | −56 … +64 | (−56,−40) (48,64) | VR5, SR7 |

At `margin = 0` the stinger-end zone covers **SR5 and SR6** — and SR6 is
precisely where the old implementation's validation work found the strain
concentration it spent effort getting right. Forcing those elements elastic
would suppress genuine plasticity exactly where the answer lives, while
still converging and still reporting a number.

The zones exist to keep boundary artefacts out of the results. That only
works if the model boundary is somewhere other than where the physics is —
i.e. the pipeline must extend **beyond** the outermost stations, with the
elastic zones living in that extension.

**Proposal: the model extends one elastic-zone length beyond the outermost
station at each end**, so `margin` defaults to `ELASTIC_END_ZONE_M` and the
zones sit outboard of every contact roller. At the defaults that gives an
extent of −56 … +64 m, 120 m of pipeline, with only the fixed station VR5
and the tension station SR7 inside a zone — both of which are boundary
...
boundary to be far from the physics. Both extend the model; they may or may
not be the same extension.

## 10. Module record (steps 5–7)

**Built:** `slay/scene/path.py` (the lay path), `slay/scene/rollers.py`
(stations and roles), `slay/scene/scene.py` (the Scene artifact).

**Diagram:** `docs/diagrams/slay_lay_layout.svg`, generated by
`tools/draw_layout.py` **from Scene**. SVG rather than mermaid, because
mermaid cannot place anything to scale and the proportions are the point.
Every coordinate comes from a Scene accessor; nothing is recomputed in the
drawing, which is tracker item 27's rule applied to a figure.

**Verification — `./tools/run_checks.sh`:** 14 modules linted clean,
97 tests pass. The §6 table:

| # | Check | Result |
|---|---|---|
| 1 | arc − \|x\| over SR1→SR6 | 1.460 m at R=85, 2.142 m at R=70 ✅ |
| 2 | SR6 geometry | θ=26.96°, s=40.0, x=−38.540, y=9.239 ✅ |
| 3 | one-sided pair position | x = +8 and +16 ✅ |
| 4 | rule scales with n_sr | ✅ — and caught a real bug, below |
| 5 | normal / tangent | (0,−1) at SR1 and on deck; tangent toward tip ✅ |
| 6 | no vessel/stinger overlap | ✅ |
| 7 | VR5 not a contact slot | ✅ |
| 8 | SR1 on the deck line | y=0; SR2 at y=0.376 ✅ |
| 9 | VR1 one spacing inboard of SR1 | ✅ |
| 10 | station counts | 12 stations, 10 contact ✅ |

**A bug the tests caught.** The first implementation defaulted the
one-sided set to `config.ONE_SIDED_ROLLERS_DEFAULT`, which is resolved once
at import against config's own `n_sr`. Building a scene with `n_sr = 9`
therefore left SR7, SR8 and SR9 **bidirectional** — silently reintroducing
exactly the staleness the rule-not-a-list design exists to prevent, by
reading the resolution instead of the rule. `config.one_sided_rollers(n_sr)`
now resolves per call, and check 4 is what found it.

**Deviation recorded:** the step-3 flowchart was replaced by the coordinate
layout, per §7. Scene has no branching worth diagramming and the layout is
what kept proving easy to get wrong.
