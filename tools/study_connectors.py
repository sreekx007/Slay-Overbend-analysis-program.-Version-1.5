...
# ---------------------------------------------------------------------------
# the prescribed-stiffness connector element -- NOW IN THE PACKAGE
# ---------------------------------------------------------------------------
#
# `connector_k6` and its zero-length branch were prototyped here and moved to
# `slay/physics/connector.py` on 14 Sep 2026. They are imported rather than
# restated, for the reason recorded as L013: a second copy of the joint
# physics is a second thing to correct, and the last one would have been
# missed when `S` was fixed.
#
# Everything the docstrings here used to carry -- why a beam matrix built at
# L0 is not self-equilibrating at L_real, why a zero-length connector is not
# the limit of a beam, which three stiffnesses the rule gives it and why the
# rotational one is EI/L0 and not 4EI/L0 -- is in that module.
from slay.physics.connector import (connector_k6, ZERO_LEN_TOL,  # noqa: E402
                                    NOMINAL_AXIS)                # noqa: F401
